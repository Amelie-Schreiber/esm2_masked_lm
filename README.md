...
- [scoring_mutations](https://github.com/Amelie-Schreiber/esm2_masked_lm/blob/main/scoring_mutations.ipynb) computes the `masked_marginal_score`, the `wild_type_marginal_score`, the `mutant_type_marginal_score`, and the `pseudolikelihood_score` for a list of mutated sequences with a fixed target mutation. 
